...
public class A_If {
	...
	public void method3() {
		...
		} else if(num < 0) { //여긴 그냥 else로 하면 좀 더 효율적이게 됨
	...
	public void method4() {
		...
		// 다른 방법으로 풀어보기 -> 어린이, 청소년, 성인에 대한 값을 담아두고
		// 마지막에 출력문으로 출력해보기
		
		String result; //박스를 만들기 (영역 밖에서 먼저 선언)
	...
	public void method5() {
		...
		String result = ""; //여기서 초기화를 안하면 else가 실행될 시 에러
		...
		System.out.printf("%s 님은 %s입니다.", name, result);
		// => 실행시 오류남. 위에 String result = ""; 로 초기화 한 번 해주는게 필수.
		
	}
	
	public void method6() {
		//특이케이스. 문자열간의 동등비교 (문자열 == 문자열)
		//사용자에게 이름을 입력받아 이규정과 일치하는지 비교하기.
		...
		//String 참조자료형이라 동등비교가 안됨(기본자료형으로 해야함)
		if(name == "이규정") {
			System.out.println("이규정님 반갑습니다.");
		} else {
			System.out.println("이규정님이 아니신가보네요?");
		}
		...
		//String을 비교하는 방법
		if(name.equals("이규정")) { //name은 클래스이기도 함 //equals는 반환값이 true, false로 boolean
			System.out.println("이규정님 반갑습니다.");
		} else {
			System.out.println("이규정님이 아니신가보네요?");
		}
	...
	public void method7() {
		...
		System.out.print("정수(양수)를 입력해주십시오 : ");
		int num = sc.nextInt();
		...
		if(num > 0) { //양수인지 확인
			...
			method7(); //메서드 내에서 다시 실행
		}
		
//		String result = "";  내가 쓰던 거 ㅋㅋ;
//		if(num%2 == 0) {
//			result = "짝수";
//		}else(num%2 == 1)
