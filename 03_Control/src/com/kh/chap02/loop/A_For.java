...
public class A_For {
	 ...
	 * 
	 * 크게 두 종류로 나뉨(FOR문, WHILE문(DO-WHILE))   //반복문의 세 종류를 말하시오 같은 문제가 나올 수 있음
	 * 
	 * [표현법]
	 * for(초기식; 조건식; 증감식){ // 세미콜론을 통해 구분. 무조건 있어야됨(세미콜론)
	 * 		반복적으로 실행시키고자하는 코드;
	 * }
	 * 
	 * - 초기식 : 반복문이 시작될 때 "초기에 딱 한번만 실행"되는 구문
	 * 		(반복문에서 사용할 변수를 선언 및 초기화할 때 사용)
	 * 		ex)int i = 0; ->계속 초기화되면 의미가 없으므로 한번만
	 * 
	 ...
	 * 		...
	 * 		-> 조건식(조건검사) -> false일 경우 실행할코드 실행 xxxx
	 * 
...
//		}
//		// 초기식, 조건식, 증감식이 없어도 문법적오류는 아니지만 무한반복으로 실행됨.
	...
	public void method2() {
		//1~5까지 출력이 되는 반복문
		// i값이 1에서부터 6보다 작을때까지 1씩 증가할동안 반복수행
		//1 2 3
		for(int i = 1; i<6; i++) {
			System.out.print(i + " ");
		}
	...
	public void method6() {
		...
		if(num > 0) {
			...
			System.out.println("1부터 " + num + "까지의 총 합계 : " + sum); //프린트문 위치!
		}else {
	...
	public void method8() { // 여기 잘 이해 안감, 설명 제대로 못들음, 랜덤값 사용하는 방법
		 ...
		 * java library 확인
		 * 
		 * static 붙은 것들은 별도 변수화가 없어도 불러와서 사용 가능?
		 * -> 객체시간에 자세히 다뤄볼 예정
		 * 
		 * random() : 0.0 ~ 0.9999999 .... 사이의 랜덤값 발생
		 * 0.0 <= 랜덤값 < 1.0
		 */
		
		//Math m = new Math(); => 사용 불가
		//m.random();   Math는 무슨 메서드?
		...
		// [표현법]
		// Math.random() * 출력을 하고자하는 갯수 + 시작하고싶은 수
	...
	public void method12() {
		...
		//단수가 2단~9단까지의 범위가 아니라면 잘못입력했습니다(2~9 사이의 정수 입력)
		
		//제대로 입력했다면 해당 단수를 출력
		Scanner sc = new Scanner(System.in);
		
		System.out.print("2~9 사이의 정수를 입력해주십시오 : ");
		int dan = sc.nextInt();
	...
	public void method13() {
		...
		for(int i = 1; i <=3; i++) {
			for(int j = 1; j <= 5; j++) { //내부 반복문의 변수는 다른것으로 지정해야함, i j k
				System.out.print(j + " ");
			}
			System.out.println();
		}
