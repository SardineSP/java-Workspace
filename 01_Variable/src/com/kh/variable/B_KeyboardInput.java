...
public class B_KeyboardInput {
	...
	public void inputTest1() {
		...
		//String name = sc.next(); => 애초에 잘 사용이 안됨
	...
	public void inputTest2() {
		...
		//문자를 뽑을 때 => sc.nextChar()? 없음!! => charAt 이 있기 때문
		
		System.out.print("이름 : ");
		String name = sc.nextLine();
		
		System.out.print("성별(M/F) : ");
		char gender = sc.nextLine().charAt(0); // index 안의 ()번째 문자를 가져온다는 뜻 >> 결국 맨 끝에 char로 반환되므로 왼쪽도 맞춰놓기
		// "apple" 문자열
		//  01234
	 ...
	 *  2. 콘솔 창(키보드)로 입력받기 위해 : Scanner을 이용해서 (nextXXX() 메서드 이용)
	 *  
	 *  	>주의 사항
	 *  	1) sc.nextXXX() 메서드 뒤에 sc.nextLine()메서드가 와야할 경우
	 *  	sc.nextLine()메서드를 한 번 더 실행시켜줘서 입력 버퍼에 남아있는 엔터를 빼줘야한다.
	 *  
	 *  	2) '문자'값을 입력받아야할 경우
	 *  		sc.nextLine()메서드를 통해 우선 문자열을 입력받고, 그 뒤에 .charAt(인덱스) 메서드를 통해
	 *  		자신이 원하는 문자를 하나만 추출한다.
	 */
