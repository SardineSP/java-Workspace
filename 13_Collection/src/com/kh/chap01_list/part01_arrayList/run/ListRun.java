...
public class ListRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 자료구조?
		 * 방대한 데이터들을 메모리상에서 다룰 때 효율적으로(구조적으로)다룰때 필요한 개념
		 * 
		 * 프레임워크
		 ...
		 * - 컬렉션의 장점
		 * 1. 크기에 제약이 없다.
		 * 		=> 크기지정을 해줄 필요도 없고 만일 크기 지정을 해놔도 알아서 크기가 늘어나면서 새로운 데이터를 추가해주는 코드가 이미 정의되어있음.
		...
		ArrayList<Music> list = new ArrayList<>(); // 내부적으로 크기 10짜리인 배열이 생성됨. (3) -> 3짜리 생성됨
		...
		list.add(new Music("Shivers", "ad sheeran"));
		list.add(new Music("villain", "stella jang"));
		list.add(new Music("RE:Wind", "isd"));
		//list.add("Good"); 해당 배열에는 Music 객체만 들어갈 수 있음
		/*
		 * 순서가 유지되면서 값 추가됨.
		 * 각 index에 담겨있음.
		 * 크기에 제약이 없음
		...
		System.out.println(list); // 원래 그 자리에 있던 값들이 한칸씩 뒤로 밀림 -> 연산속도가 느려짐
		
		//3. set(int index, E e) : 리스트에 원하는 위치의 인덱스값을 제시하면 전달하는 e값으로 변경해주는 메서드
		list.set(0, new Music("sugar", "bts")); // 바로 그자리에 박아줌 -> 연산속도가 add 보다는 빠름
...
//		System.out.println(m.getTitle());
		...
		//7. subList(int index1, int index2) : 해당 리스트로부터 index1에서 index2까지의 데이터값들을 
		...
		//9. isEmpty() : 해당 리스트가 비어있는지 묻는 메서드(비어있으면 true/ 비어있지 않다면 false 반환)
