...
public class Run {
	...
	public static void main(String[] args) {
		...
		// 값 추가 : add(추가할 값);
		hs1.add(new String("반갑습니다.")); // 이렇게 String 생성자로 넣어주면 heap영역 안에 각각의 주소값으로 들어가게 됨
		hs1.add(new String("반갑습니다."));
		hs1.add(new String("여러분"));
		hs1.add(new String("안녕하세요."));
		hs1.add(new String("여러분"));
		//hs1.add(50); -> 내가 제네릭 설정으로 설정한 자료형만 넣을 수 있음
		...
		// 값 삭제 : remove(삭제할 값) -> 인덱스가 없으므로 값 그자체를 제시해줘야함
		hs1.remove("여러분");
		...
		hs2.add(new Student("이규정", 27, 100));
		hs2.add(new Student("홍길동", 31, 90));
		hs2.add(new Student("김갑생", 50, 70));
		hs2.add(new Student("이규정", 27, 100));
		
		System.out.println(hs2); // 저장 순서 유지도 안되고 동일한 값이 중복으로 들어감 -> 동일객체로 판단되지 않기 때문
		...
		// set => 무작위로 객체가 저장되어있음. (index개념이 없음) => 반복문 활용이 불가
		...
		for(int i = 0; i<list.size(); i++) { // -> 인덱스가 생겼으므로 가능해짐
		...
		while(it.hasNext()) { // it에 next로 빼올 값이 있는가? 라고 물어보는 메서드
			it.next();
		} // String의 StringTokenizer과 비슷한 개념.
