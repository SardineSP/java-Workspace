...
public class A_Array {
	 ...
	 *   stack 영역에 저장		heap 영역	   //방, 사물함 [0][1][2]을 3개 만듬.
	 * 								   //인덱스는 항상 0부터 시작한다.(중요)
	 * 								   //배열의 크기는 3, 마지막 인덱스 = 2
	 * 								   //배열의 크기 100, 마지막 인덱스 = 99 (무조건 배열의 크기 -1이므로)
	 * 								   //항상 크기 = n, 마지막 인덱스 = n-1
	 * 
	 */
	...
	public void method1() {
		//배열을 왜 써야할까
		//만약에 변수만을 가지고 프로그래밍을 하게되면 0, 1, 2, 3 기록해야함
		 ...
		 * 
		 * for(int i = 1; i<=5; i++){
		 * 		System.out.println(num+i); //절대 안됨 ㅋㅋ
		 * }
		 * 
		 * 1. 배열을 가지고 프로그래밍하기
		 * 0, 1, 2, 3, 4
		...
		//System.out.println(arr[0]); //각 자료형의 기본값으로 초기화해줌?
		...
		//각각의 인덱스값을 확인하기 위해서는 []블럭을 통해 내가 확인하고자하는 인덱스(방번호)를 제시해줘야함.
		//그럼 arr에 전체 담겨있는 값을 확인하고 싶다면?
		System.out.println(arr); //[I@1888ff2c -> 배열 int @ 주소값
		//실제 arr이라는 값에는 주소값이 담겨있음.
		// [ : 배열임을 나타냄
		// I : int 자료형임을 의미함
		// @ 기준 우측은 참조하고 있는 heap 영역의 주소값
		
		//array 내부 값을 출력하는 방법은 추후 배움 
	}
	...
	public void method2() {
		 ...
		 * 그 다음 int[5]짜리 방이 heap영역에 생성됨.(new 키워드가 붙은 녀석은 항상 heap 영역에 값이 할당됨)
		 * 5칸짜리 방이 만들어진 후에 각방에 기본값으로(int 0으로) 데이터가 초기화되고, 주소값이 할당됨 (0x123 등)
		 * 마지막으로 대입연산자가 실행되면서 주소값이 arr에 할당이 됨.
		 * 
		 */
		System.out.println("i : " + i);
		System.out.println("arr : " + arr); //주소값
		System.out.println("arr의 해시코드값 : " + arr.hashCode()); //주소값을 10진수로 바꾼 것
	...
	public void method4() {
		...
		for(int i = 0; i < arr.length; i++) {
			arr[i] = i + 1; // arr[0], arr[1], arr[2]... 모든 방에 접근 가능
		}
		
		System.out.println(arr[4]);
		/* System.out.println(arr[5]);
		 * ArrayIndexOutOfBoundsException => 인덱스 길이 초과
		 * 배열의 인덱스 범위를 벗어남.
		 */
	}
	...
	public void method5() {
		...
		System.out.println(arr.hashCode()); // 지금 arr 주소값은 null이라 error
		/*
	...
	public void method6() {
		 ...
		 * => 배열의 크기를 변경하고자 한다면 어떨 수 없이 새로운 배열을 다시 만들어야함.
		 * 
		 */
		...
		System.out.println(arr);
		System.out.println("arr의 해시코드값 : " + arr.hashCode()); //이부분 놓친거 있는지 보기
		/*
	...
	public void method7() {
		...
		//배열 비교
		System.out.println(arr == arr2); //System.out.println(5 == 2); 이것도 false로 되네?
		//arr1 -> arr1의 heap영역의 "주소값"
		//arr2 -> arr2의 heap영역의 "주소값"
		// 주소값 == 주소값 : 주소값간의 동등비교를 수행하기 때문에 무조건 false
	}
	...
	public void method8() {
		...
		int[] arr = new int[10];
		for(int i = 0; i < 10; i++) { //i < arr.length;
			arr[i] = (int)(Math.random()*100 + 1);
			System.out.printf("arr[%d] : %d\n", i, arr[i]); // 이걸 굳이 왜 나눴을까? arr.length 때문?
		}
	...
	public void method10() {
		//사용자에게 문자열을 하나 입력받은 후, 각각의 문자들을 char 배열에 옮겨담기
		
		//1. 사용자에게 문자열 입력받기
		Scanner sc = new Scanner(System.in);
		System.out.print("문자열 입력 : ");
		String text = sc.nextLine(); // 받아온 문자열을 저장
		...
		//4. char 배열의 각 인덱스에 있는 값들 출력하기
			System.out.print(txt[i] + " "); // 왜 또 나누지? -> 이렇게 해도 됨!
		}
	...
	public void method11() {
		...
		//3. 반복문을 활용해서 0번 인덱스~ 마지막 인덱스까지 배열에 담긴 값 출력과 동시에 
	...
	public void method12() {
		...
		for(int i = 0; i < 3; i++) {
			System.out.printf("%d번째 키를 입력해주십시오(cm) : ", i + 1);
			heights[i] = sc.nextInt();
			sum =+ heights[i];
		}
		System.out.printf("3명의 키의 총합은 %.1fcm입니다.", sum); // %0.1f 는 안됨!!
		System.out.printf("3명의 키의 평균은 %.1fcm입니다.", sum / 3);
//		풀이
