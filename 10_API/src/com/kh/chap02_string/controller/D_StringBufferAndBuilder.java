...
public class D_StringBufferAndBuilder { // 이거 잘 모르겠음... -> 원본 객체 자체에 변화를 주는 메서드?
	/*
	 * String은 불변클래스,
	 * StringBuffer과 StringBuilder은 mutable클래스
	 * 
	 * 문자열 연산이 빈번하게 일어나는 알고리즘에서 발생하는 메모리 이슈를 해결하기 위해 나온 클래스
	 * 기본적으로 두개의 클래스는 동일한 메서드를 지니며 차이점은 동기화(스레드safe) 여부.
	 * 
	...
	public void method() {
		...
		StringBuffer sb = new StringBuffer("Hello"); //주소값 찾아가는것?
		sb.append("World,");
		// 1 , 2 , 3 , 4 , 5
		
		StringBuilder sb2 = new StringBuilder("Hello"); //하나에서 계속 더하거나 빼거나?
		sb2.append("World");
