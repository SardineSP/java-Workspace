...
public class Run {
	 ...
	 * 			외부로부터 '데이터의 접근을 막고' 대신에 '데이터를 간접적으로나마 처리'할 수 있는 메서드를
	 * 			클래스 내부에 작성해서 관리하는 방식.
	 * 		캡슐화를 하지 않으면 : 외부로부터 직접 접근이 가능하기 때문에 함부로 값이 변질되거나 조회를 막지 못하는 문제 발생
	 * 
	 * 		1) 정보은닉 : private
	 * 			필드들을 외부로부터 직접접근을 막기 위해 public 대신에 private 접근제한자 사용
...
//		jung.name = "이규정"; // The field Student.name is not visible
//		jung.age = 26;
