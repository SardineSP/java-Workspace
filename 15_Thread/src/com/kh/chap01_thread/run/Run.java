...
public class Run {
	 ...
	 * 2) 사용자 입장에서 일처리가 빠르게보인다.
	 * 3) 작업이 분리되어 코드가 간결해진다. -> 많으면 많을수록 단점이 됨
	 * 
	 * ex) 
	 * - 청소(나 -main), 빨래(세탁기 -sub), 설거지, 화장실청소
	 * - 무조건 주체가 되는, 중심이 되는 스레드가 메인스레드 == 자바의 메인메서드
...
//		}
		// 현재 작업으로 멀티스레드 환경을 만드려면 필요한 스레드의 개수 -> 1개 더
		...
		// 스레드 생성방법 : 2. Runnable 인터페이스를 구현하기. -> 권장
		Thread th2 = new Thread(new Thread2()); // 이거 잘 모르겠음
		
		th2.setName("스레드2");
		
		th2.start();
		
		// 스레드 생성방법3 : 1회용 스레드가 필요할 때 (익명클래스)
