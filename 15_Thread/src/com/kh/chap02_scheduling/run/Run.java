...
import com.kh.chap02_scheduling.scheduling.Airplane;
...
public class Run {
	...
	public static void main(String[] args) {
		...
		Thread t2 = new Thread(new Airplane());
		 ...
		 * 
		 * 주종관계 설정
		 * main 스레드가 종료시 다른 스레드도 종료시키기 위해서 반드시 setDaemon사용.
		 * 주의점은 스레드중 한개라도 setDaemon으로 설정시 모든 스레드에 setDaemon설정을 해줘야함
		 */
		System.out.println("메인스레드의 끝부분");
		// 시작순서는 우선순위가 다를 수 있지만 종료순서는 우선순위가 높은 순으로 끝나게됨(항상 그렇지는 않음).
