...
public class PolyRun {
	...
	public static void main(String[] args) {
		...
		p1.printParent();
		//p1레퍼런스 변수로 Parent에만 접근 가능
		
		//2. 자식타입 레퍼런스 변수로 자식 객체를 다루는 경우? (지난시간에 배운 것)
		System.out.println("2. 자식타입 레퍼런스변수로 자식객체를 다루는 경우");
		Child1 c1 = new Child1(); // 기본생성자에 슈퍼생성자 호출하는 구문이 항상 있음. 따라서 부모객체 사용 가능.
		c1.printChild1();
		((Parent)c1).printParent(); // 좌측과 같이 업캐스팅이 자동으로 이루어지고 있음
		//c1 레퍼런스로 Parent(부모), Child(자식)에 있는 필드나 메서드 둘 다 접근가능.
		//c1 레퍼런스변수가 Parent에 있는 메서드에 접근 시 자동으로 형변환된채 진행됨.
		//즉 자식에서 부모클래스 자동형변환(업캐스팅) -> 작은것에서 큰것으로 형변환된 것과 같은 이치
		
		//3. 부모타입 레퍼런스변수로 자식 객체를 다루는 경우 (다형성이 적용된 개념)
		System.out.println("3. 부모타입 레퍼런스변수로 자식 객체를 다루는 경우 (다형성 적용된 개념)");
		Parent p2 = /*(Parent)*/ new Child1(); 
		/*
		 * 양쪽의 자료형이 다른데도 불구하고 오류 발생 x
		 * Parent형으로 '자동형변환'이 되고있기 때문에
		...
		//단, 다형성이 적용되면 부모타입레퍼런스 변수로 모든 자식객체를 다 받아줄 수 있다.
...
//			}
			...
			arr[i].print();
			/*
			 * 동적바인딩 : 프로그램이 실행되기 전에 자동으로 컴파일되면서 정적으로 현재 레퍼런스 변수의 메서드를 가리키게 됨
			 * 단, 참조하고 있는 자식 클래스에 해당 메서드가 오버라이딩되어있다면
			 * 프로그램을 시작할때는 동적으로 그 자식클래스의 오버라이딩된 메서드를 찾아서 실행하게됨
			 * (ctrl 누르고 들어가보면 Parent 클래스의 메서드를 실행한다고 했지만(정적바인딩),
			 * 실제로 실행시에는 자식클래스의 오버라이딩된 메서드가 실행됨.(동적바인딩). 
			 * 
			 */
		}
